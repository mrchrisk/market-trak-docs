...
Click the **Share to Social** button in the report detail view. This opens the calendar's post composer in **From a report** mode with your report already selected, so its data (average price, price trend, demand rating, days on market) flows into the caption.
</Step>
<Step title="Generate an image" icon="image" titleType="p">
Pick an image style and aspect ratio (square for Instagram, landscape for Facebook), then generate. Market Trak creates a professional AI-generated image using your report data. Want a different look? Go back and pick another style for a fresh image.
</Step>
...
<Callout kind="info">
There is nothing to set up before the assistant writes for you, and no writing style to pick. Every post comes out at one sensible length, and if you want it different, just say so: "make that shorter", "give me a bit more detail". Your standing instructions in **User Settings > AI Assistant** still shape the tone of everything it writes.
</Callout>
...
**The guided flow** (the calendar post composer or Property Feed) gives you a structured, step-by-step process. You pick a report, choose a caption style from the templates, generate an image, and publish. It is quick and consistent. Good for when you know what you want and just need to get it done.
...
For each new listing you get three posts, each written from a different angle and each using a different photo from the listing, spread across the following week so one property keeps working for you rather than going out once and being forgotten.
...
<Expandable title="Why do I get three posts for one listing?" default-open="false">
They are not three versions of the same thing to choose between. All three publish, spread across the following week, so a new instruction gets more than one moment in front of your local audience.

Each one takes a different angle on the same home: one leads on what living there would feel like, one on the detail that sets it apart, one on a single striking fact. They are the same length as each other, so the set reads as deliberate, and each uses a different photo from the listing. The first two close by inviting a message or a viewing; the last one carries the link to the full listing.

You can edit or delete any of them in your [Approval Queue](/help-center/social-media/approval-queue) before they go out.
</Expandable>
