...
You can generate a full market report straight from the chat. Ask for one, for example "Generate a market report" or "Create a report for CM15", and the assistant walks you through it:

1. **Enter the location.** Type a postcode in the box that appears. Just like the Generate Report form, it nudges you toward the formats that return the best data: a tip shows if you type a full unit-level postcode (suggesting the shorter outward code, such as CM15) or a town name (suggesting a postcode instead).
...
You can create a whole [Magic Pack](/help-center/magic/magic-packs) without leaving the chat. A Magic Pack is a one-click bundle: a fresh local market report plus three ready-to-post social posts for a postcode.

1. Ask for one: "Make me a Magic Pack", "Create a report and some social posts for CM15", or "I need a starter pack for NW6"

2. A postcode box appears right in the chat. If you already named a postcode, it is pre-filled, so you just confirm

3. Click **Generate**. A compact progress animation runs while the pack is built (around ninety seconds)

4. When it finishes, a link appears that takes you straight to your finished pack

The assistant builds the pack the same way the Magic page does, so it uses the same credits and the same monthly pack allowance; nothing is charged twice. The postcode box only accepts real postcodes. If you type a town name like "Chelmsford" it nudges you to use a postcode instead, because postcodes return more accurate area data.

If you open your pack and then come back to the chat, the link is still there waiting for you. You can edit, save, schedule, and publish the posts from the pack itself, exactly as you would from the Magic page.
