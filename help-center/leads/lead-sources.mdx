...
If you want visitors to land on your own page first, paste this once into that page, just before the closing `</body>` tag. It copies the ad's tags onto any link that points to Market Trak, so the tracking rides through.
</Step>
</Steps>

```html
<!-- Market Trak: carry ad tags onto links to Market Trak -->
<script>
(function () {
  ...
  function decorate(a) {
    try {
      ...
      if (!/(^|\.)marketrak\.pro$/i.test(url.hostname)) return;
...
<Callout kind="info">
**Using the embedded form instead of a button?** The valuation and signup embeds already do this for you. Just re-copy the latest embed code from your dashboard, and keep the `<script>` part that comes with it.
</Callout>
